...
> **发布状态 / Release status**
>
> `v0.2.0` 正式版本现已发布，可从本仓库的 Releases 页面下载。
>
> The `v0.2.0` release is now available from this repository's Releases page.

### v0.2.0 更新

- 全面调整首页、说明弹窗与清洁过程界面，统一排版、交互反馈和动画
- 更新应用图标，并优化 Dock 中的显示效果
- 窗口固定为 760 × 560，不再支持缩放或全屏，避免布局异常
- 加固清洁会话恢复机制，覆盖启动失败、超时、应用退出、睡眠和会话切换等情况
...
1. 从本仓库的 [Releases](https://github.com/b0vibe/mac-clean-helper-releases/releases) 页面下载 `Mac-Clean-Helper-v0.2.0-macOS-arm64.zip`。
2. 使用同一 Release 中的 `SHA256SUMS.txt` 核对下载文件。
3. 解压 ZIP，将“Mac 清洁助手”拖入“应用程序”文件夹。
4. 从“应用程序”正常打开软件。
5. 按软件引导前往“系统设置 → 隐私与安全性 → 辅助功能”授权。

请下载名称为 `Mac-Clean-Helper-v0.2.0-macOS-arm64.zip` 的应用安装包。Release 页面底部由 GitHub 自动生成的 `Source code (zip)` 和 `Source code (tar.gz)` 只包含本公开仓库的说明文档，不是应用安装包，也不是本应用的源代码。
...
- Redesigned the home screen, guidance dialogs, and active-cleaning views with consistent layout, interaction feedback, and motion
- Updated the app icon and refined its appearance in the Dock
- Fixed the window at 760 × 560 and disabled resizing and full screen to prevent layout issues
- Hardened cleaning-session recovery for startup failures, timeouts, app termination, sleep, and session changes
...
1. Download `Mac-Clean-Helper-v0.2.0-macOS-arm64.zip` from this repository's [Releases](https://github.com/b0vibe/mac-clean-helper-releases/releases) page.
...
Download the application package named `Mac-Clean-Helper-v0.2.0-macOS-arm64.zip`. The `Source code (zip)` and `Source code (tar.gz)` files automatically generated by GitHub at the bottom of the Release page contain only this public repository's documentation. They are not application packages and do not contain the app's source code.
